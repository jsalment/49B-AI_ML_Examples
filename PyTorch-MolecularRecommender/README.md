...
```
git clone https://github.com/joseph-salmento_usma/Molecular_Recommender
```
...
### Training
The recommender system builds on to of the transformer embeddings. It creates an embidding for the target molecule and the nucleotide molecule and trains a neural network to predict an affinity score between the nucleotide and the target.

Use the following command to train the recommender
```
python recommender_train.py
```

Optional arguments for training
```
usage: recommend_train.py [-h] [--data_path DATA_PATH] [--checkpoint_path CHECKPOINT_PATH] [--embedding_size EMBEDDING_SIZE]
                          [--num_layers NUM_LAYERS] [--b_thres B_THRES] [--a_thres A_THRES] [--s_thres S_THRES]

options:
  -h, --help            show this help message and exit
  --data_path DATA_PATH
                        Path to a csv file with nucleotide SMILES, target SMILES string and Affinity score per line. These pairs will form the
                        recommender.
  --checkpoint_path CHECKPOINT_PATH
                        Path to a binary file containing pretrained model weights.
  --embedding_size EMBEDDING_SIZE
                        Embedding size used in the pretrained Transformer.
  --num_layers NUM_LAYERS
                        Number of layers used in the Encoder and Decoder of the pretrained Transformer.
  --b_thres B_THRES     Threshold below which an affinity score will assumed bind with a target molecule.
  --a_thres A_THRES     Threshold above which an affinity score will be removed from dataset
  --s_thres S_THRES     Threshold above which an affinity score will be removed from dataset
```

### Prediction
The recommender system takes in a target molecule returns the top k nucleotide sequences based on the predicted affinity scores for the molecule.

Use the following command to use the recommender for prediction.
```
python recommender_pred.py
```

Optional arguments for prediction
```
usage: recommend_pred.py [-h] [--data_path DATA_PATH] [--checkpoint_path CHECKPOINT_PATH] [--k K] [--target TARGET]
...
options:
  ...
  --checkpoint_path CHECKPOINT_PATH
                        Path to a binary file containing pretrained model weights.
  --k K                 Number of molecules to recommend.
  --target TARGET       Target molecule to provide recommended Oligos for.
```
...
## Citing
# Recommender
Recommender not yet published.

# Original Paper on Embeddings
