...
parser = argparse.ArgumentParser(description="Compute median of 7th numeric column from .out files")
parser.add_argument("--in_dir", default='data/MissingSpecificityOutFolder', help="Directory containing .out files")
parser.add_argument("--out_path", default="data/median_out_large.csv", help="Output CSV path")
args = parser.parse_args()

# regex: floats and ints, including scientific notation
num_re = re.compile(r'[-+]?\d*\.\d+(?:[eE][-+]?\d+)?|[-+]?\d+(?:[eE][-+]?\d+)?')

def median_of_7th_from_file(path):
    vals = []
    ...
    if len(vals) == 0:
        return float("nan")
    # Use np.median for robustness (handles empty lists safely too, but we already checked)
...
for p in file_paths:
    ...
    median7 = median_of_7th_from_file(p)
    results.append({"Filename": fname, "Median7": median7})
